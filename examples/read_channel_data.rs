...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file_path = args.get(1).map(|s| s.as_str()).unwrap_or("tests/fixtures/tdms_corpus/03_datatypes/floats.tdms");
    ...
    println!("Target: Group '{}', Channel '{}'", target_group, target_channel);
    
    let file = TdmsFile::load(Path::new(file_path))?;
    
    // Find the specified group
    let group = file.groups.get(target_group)
        .ok_or_else(|| format!("Group '{}' not found", target_group))?;
    
    // Find the specified channel
    let channel = group.channels.get(target_channel)
    ...
    match data {
        ...
        TdmsData::Boolean(values) => {
            ...
            if !values.is_empty() {
                ...
                println!("  True: {} ({:.1}%)", true_count, 100.0 * true_count as f64 / values.len() as f64);
                println!("  False: {} ({:.1}%)", false_count, 100.0 * false_count as f64 / values.len() as f64);
