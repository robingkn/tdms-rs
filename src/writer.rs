    ...
    fn build_segment_data(&self) -> Result<(Vec<u8>, Vec<u8>)> {
        ...
        for (group_name, group) in &self.groups {
            for (channel_name, channel) in &group.channels {
                ...
                let raw_data_info = self.build_raw_data_info_with_properties(&channel.data, &channel.properties)?;
                ...
                self.write_channel_object_metadata(&mut metadata, &path, raw_data_index, &raw_data_info, &channel.properties, is_string)?;
...
mod tests {
    ...
    fn properties_support() -> Result<()> {
        ...
        assert_eq!(test_group.properties.get("group_prop"), Some(&PropertyValue::String("group_value".to_string())));
        assert_eq!(test_group.properties.get("group_double"), Some(&PropertyValue::Double(3.14)));
        
        // Verify channel properties
        let test_channel = test_group.channels.get("TestChannel").unwrap();
        assert_eq!(test_channel.properties.get("wf_unit_string"), Some(&PropertyValue::String("V".to_string())));
        assert_eq!(test_channel.properties.get("wf_increment"), Some(&PropertyValue::Double(0.001)));
        assert_eq!(test_channel.properties.get("channel_bool"), Some(&PropertyValue::Boolean(true)));
    ...
    fn examine_strings_corpus() -> Result<()> {
        ...
        let reference_file = TdmsFile::load(std::path::Path::new("tests/fixtures/tdms_corpus/03_datatypes/strings.tdms"))?;
    ...
    fn boolean_and_timestamp_data() -> Result<()> {
        ...
        match &timestamp_channel.data {
            Some(TdmsData::TimeStamp(written_timestamps)) => {
                assert_eq!(written_timestamps, &vec![
                    (1000, 0),
                    (2000, 500000000),
                    (3000, 1000000000),
                ]);
    ...
    fn round_trip_strings_corpus() -> Result<()> {
        ...
        let written = TdmsFile::load(std::path::Path::new(output_path))?;
        let reference = TdmsFile::load(std::path::Path::new("tests/fixtures/tdms_corpus/03_datatypes/strings.tdms"))?;
        
        // Compare structure
        assert_eq!(written.groups.len(), reference.groups.len());
        assert_eq!(written.groups.keys().collect::<Vec<_>>(), reference.groups.keys().collect::<Vec<_>>());
        ...
        match (&written_channel.data, &reference_channel.data) {
            (Some(TdmsData::String(written_strings)), Some(TdmsData::String(reference_strings))) => {
